...
\begin{frame}{목차}
  ...
  \begin{itemize}
    \item \textbf{고전 ML과 신경망의 역사}: 회귀(1805)$\cdot$SVM(1995)의
      ...
      ``왜 하필 2012년이었나''를 \kb{계산}과 \kb{데이터} 두 축으로.
...
\begin{frame}[plain]
  \centering
  \includegraphics[width=0.92\linewidth]{w01_p2.jpg}
  \vskip0.3em
  {\small ImageNet (CVPR, 2009) \\[0.2em] ILSVRC Challenge (2010$\sim$, 1000 categories)}
\end{frame}
...
\begin{frame}{이 과목이 다루는 질문}
  ...
  \vskip1em
  \begin{center}
  \kq{규칙을 사람이 짜는 대신, 정답이 있는 데이터로부터\\
      기계가 직접 규칙을 찾아내게 하려면 어떻게 해야 할까?}
  \end{center}
\end{frame}

\begin{frame}{고전 ML과 신경망 --- 두 갈래의 역사}
  \small
  \begin{itemize}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \centering{\footnotesize\textbf{고전 ML (통계적 학습) --- Ch.\,2$\sim$7}}\\[0.2em]
      \scriptsize
      \begin{tabular}{@{}ll@{}}
        \toprule
        연도 & 사건 \\
        \midrule
        1805 & 최소제곱법(르장드르$\cdot$가우스) --- 회귀의 기원 (Ch.\,2) \\
        1936 & 피셔의 선형판별분석(LDA) (Ch.\,3) \\
        1951 & 최근접이웃 규칙(Fix\&Hodges) $\to$ kNN (Ch.\,4) \\
    ...
    \begin{column}{0.5\textwidth}
      \centering{\footnotesize\textbf{신경망 (퍼셉트론$\to$LLM) --- Ch.\,9$\sim$17}}\\[0.2em]
      \scriptsize
      \begin{tabular}{@{}ll@{}}
        \toprule
        연도 & 사건 \\
