...
To feed an image into the Multi-Layer Perceptron (MLP) from Chapter 8, you'd
...
cells to complex cells, that Hubel and Wiesel observed. How this structure
gets applied to real problems (detection, segmentation) is covered in
Chapter 11.
...
**2. (Conceptual)** Holding the input size and filter size fixed, explain
using the output-size formula how the output size and computation (FLOPs)
change if you increase the stride \\(s\\) from 1 to 2.

**3. (Hand derivation, Tier A — free derivation)** For an input of size
