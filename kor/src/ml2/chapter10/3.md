...
REINFORCE는 \\(G\_t\\)(실제로 관찰된 누적 보상)를 그대로 쓰는데, 이건
하나의 에피소드를 샘플링한 결과라 노이즈가 크다(분산이 높다).
**Actor-Critic**은 \\(G\_t\\) 대신, Chapter 4·6에서 배운 가치함수
...
10.2절의 CartPole REINFORCE는 처음 20 에피소드 평균 14.1에서 마지막
...
내준다는 Critic(값을 예측하는 출력층)을 하나 더 붙인다. PyTorch로
쓰면:
...
**A**ctor-**C**ritic(DeepMind, 2016) — "A"가 바로 오늘 배운
...
나왔다. Schultz, Dayan, Montague는 원숭이의 뇌에서 **도파민
...
일화 중 하나다. (11.3절의 GAE도 이 TD 오차 \\(\delta\_t\\)를
여러 개 가중평균한 것 — 같은 뿌리다.)
...
Chapter 9의 DQN이 더 데이터 효율적일 때가 많다. Actor-Critic이
...
사용할 때까지 이 구조를 *실무적으로* 익숙해져 있어야 한다.
