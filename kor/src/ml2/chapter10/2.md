...
![CartPole에서 REINFORCE 300 에피소드 학습 곡선. 연회색은 에피소드별 리턴, 파란 실선은 20 에피소드 이동 평균, 빨간 점선은 만점 500. 이동 평균은 약 13에서 약 100으로 상승하며, 개별 에피소드는 큰 변동(노이즈)을 보인다 (원 논문: Sutton 1984, CartPole 정책 그래디언트 실험)](../../images/ch10_2_reinforce_cartpole_curve.svg)
...
장(Actor-Critic, PPO[^ppo][^trpo][^gae])이 정확히 이 문제를 개선하려는 시도다.
...
그 무렵 Williams(1992)[^williams92]는 이 갱신 규칙이 통계학의 고전적
기법인
