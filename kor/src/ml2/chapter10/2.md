...
softmax. 신경망은 등장하지 않는다 — 위 PyTorch 실습[^pytorch]의
...
### 실습: PyTorch로 REINFORCE를 CartPole[^gym]에서 학습시키기
...
![CartPole에서 REINFORCE 300 에피소드 학습 곡선. 연회색은 에피소드별 리턴, 파란 실선은 20 에피소드 이동 평균, 빨간 점선은 만점 500. 이동 평균은 약 13에서 약 100으로 상승하며, 개별 에피소드는 큰 변동(노이즈)을 보인다 (원 논문: Sutton 1984, CartPole 정책 그래디언트 실험)[^suttonbarto]](../../images/ch10_2_reinforce_cartpole_curve.svg)

Chapter 9.3의 DQN[^dqn] (같은 300 에피소드에서 마지막 20 에피소드 평균
...
장(Actor-Critic, PPO[^ppo])이 정확히 이 문제를 개선하려는 시도다.
...
![3D 워커 환경 학습 곡선 (원 논문 Figure 1) — PPO+GAE(ours, 청색)가 DDPG/TRPO/SAC 대비 더 적은 반복 횟수로 높은 보상을 달성함을 보여준다[^gae].](../../images/ref_gae.png)
...
Sutton의 1984년 MIT 박사 학위논문[^suttonbarto]에는 이미 "행동의 결과를
...
것"이라는 사실의 기준점이 되어서 중요하다.[^cs234][^silvercourse]
