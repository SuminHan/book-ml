...
\\(\\varepsilon\\)-greedy와 UCB로 그 딜레마를 처음 풀 때 마찰 없이 들어갈 수
있다.
...
- "강화학습의 새로움은 학습 알고리즘이 아니라 손실함수의 정의에 있다"는
  관점에서, DQN(Ch. 9)이나 PPO(Ch. 10~11)에서도
  `zero_grad()` → `backward()` → `step()` 세 줄이 그대로 쓰이는 이유를
  미리 설명할 수 있다.
- Gymnasium의 `reset`/`step` 표준 계약과 관측·행동 공간(`Box`/`Discrete`),
...
- [1.1 강화학습이란 무엇인가, 그리고 이번 학기 로드맵](chapter01/1.md) —
  ...
  특허부터 DQN·PPO·AlphaGo·RLHF까지의 역사를 "이번 학기의 어느 장에
