...
> 강화학습과도 직결된다. 정책 그래디언트로 "어떤 토크가 좋은가"를
...
무료로 오픈소스화하면서, GPU 병렬 시뮬레이터(14.2의 Isaac Sim)가
없어도 CPU에서 정밀 로봇 물리를 돌릴 수 있게 돼, OpenAI의
`gym`에서 DeepMind의 로봇 연구까지 사실상 표준 물리엔진이 됐다.
...
있습니다. (1) 보상/저장의 스케일이 다르다 — PPO의 GAE·어드밴티지
