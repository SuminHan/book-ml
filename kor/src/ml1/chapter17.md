# Chapter 17 (부록). LLM: 사전학습, 프롬프팅, 정렬 (LLM: Pretraining, Prompting & Alignment)
...
"다음 토큰의 확률분포"를 계산하는 실제 엔진이 된다.

다만 한 가지 위치를 분명히 해 두자. 이 장은 1~16장 핵심 커리큘럼
이후에 읽는 **심화·응용 부록**이다 — 즉, Chapter 14(표현학습)에서
word2vec[^word2vec]·Node2Vec[^node2vec]·PageRank가 "단어·노드·데이터를 벡터로
표현한다"는 질문을 정면에서 다뤘고, Chapter 13(그래프 표현학습)에서
그 아이디어를 실제 그래프 학습까지 밀어붙인 *뒤*에, 그 도구들로 이미
구축된 "대형 모델"을 학습하고 다루고 다듬는 방법을 한 장에 모은
자리다. 앞서 Chapter 14에서 만난 임베딩은 이 장에서 다시 등장한다 —
LLM이 문맥마다 다른 의미를 고차원 벡터로 압축하는 그 임베딩과 같은
뿌리에서 자라났기 때문이다.
...
- [17.1 언어모델링: 다음 토큰 예측과 사전학습·파인튜닝](chapter17/1.md)
...
- [17.2 프롬프팅](chapter17/2.md)
  ...
  원인까지 함께 다룬다.
- [17.3 포스트트레이닝: SFT, RLHF, DPO](chapter17/3.md)
