...
[STUB: cq가 확장할 자리 — 아래는 핵심 뼈대(기존 오프너에서 Node2Vec/
PageRank 관련 문단만 들어냄). 14.1/14.2 문단은 원래 내용 재사용,
14.3 문단만 새로 씀. **주의**: 13장(그래프 표현학습)이 이 장보다
먼저 나오는데 13.2(Node2Vec)가 이 장 14.2(word2vec)의 skip-gram을
전제로 설명하게 되어 있다 — 순서상 앞뒤가 안 맞는다. cq 확장 시
(a) 13.2에서 skip-gram을 자기완결적으로 짧게 재설명하거나, (b) 이
문제를 사용자에게 다시 확인할 것.]
...
그 아이디어의 원형(텍스트 버전)이다. 12장에서 배운 Transformer의 입력층
