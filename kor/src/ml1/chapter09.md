# Chapter 9. 딥러닝 학습 기법 (Deep Learning Training Techniques)
...
**2. (개념 서술)** Batch Normalization과 Dropout은 둘 다 정규화 효과를
내지만 작동 방식이 다르다. 추론(inference) 시점에 두 기법이 각각 어떻게
동작이 달라지는지(학습 때와 비교해서) 설명하라.

**3. (손유도, Tier B — 힌트 제공)** 시그모이드 \\(\sigma(z) =
