...
| 1969 | 민스키·페퍼트가 단층 퍼셉트론이 XOR을 못 푼다는 것을 증명[^minskypapert] → 첫 "AI 겨울" (Chapter 9) |
| 1986 | 역전파(backpropagation)[^rwh86] 재발견으로 다층 신경망 학습이 가능해짐 (Chapter 9) |
| 1998 | LeCun의 CNN(LeNet)[^lenet]이 손글씨 숫자 인식에 성공 (Chapter 10) |
| 2012 | AlexNet[^alexnet]이 ImageNet[^imagenet]에서 압승 — 딥러닝 시대의 시작 (이 챕터의 도입부) |
| 2013 | word2vec[^word2vec], VAE[^vae] 등 표현학습·생성모델의 현대적 형태 등장 (Chapter 14, 15) |
| 2017 | Transformer("Attention Is All You Need")[^transformer] 등장 (Chapter 12) |
| 2022 | ChatGPT — 사전학습 + RLHF[^rlhf][^instructgpt]로 조정된 대규모 언어모델의 대중화 (Chapter 17) |
...
Block A에서 다루는 고전 ML 방법론을 더 깊이 보려면: [^cs229].
...
비지도학습(PCA, 임베딩)과 잠재변수 기반 생성형 모델(EM/GMM에서 VAE·GAN[^gan]·
Diffusion[^diffusion]까지)로 학기를 마무리한다(Chapter 14~15). Chapter 16은 두 번째
팀 프로젝트 발표와 학기 총정리다.
...
| 4 | 거리 기반 모델과 클러스터링: kNN, k-means, GMM/EM | A |
| 5 | SVM과 커널: 마진 최대화, 소프트 마진, 커널 트릭 | A |
| 6 | 정규화와 모델 선택: L1/L2, 교차검증, train/val/test 분리 | A |
| 7 | 트리 기반 모델: 결정트리, 랜덤 포레스트, GBDT와 SHAP[^shap] | A |
...
| 13 | 그래프 표현학습: Random Walk, Node2Vec[^node2vec], PageRank | B |
| 14 | 표현학습: PCA, word2vec, 임베딩의 활용(Transformer·RAG) | B |
| 15 | 잠재변수 생성모델: VAE, GAN, Diffusion | B |
| 16 | Block B 캡스톤: 팀 프로젝트, Peer-Review, ML1 총정리 | B |
| 17 (부록) | LLM: 언어모델링, 프롬프팅, SFT·RLHF·DPO[^dpo] | — |
...
|---|---|---|
| numpy[^numpy] (N-dimensional array) | 모든 실습의 기본. \\(w^Tx\\), 행렬곱, 경사하강법 반복 | Chapter 2 |
| matplotlib (plotting) | 손실 곡선, 결정경계, t-SNE/PCA 시각화 | Chapter 2 |
| pandas (table data) | EDA, 결측치 처리, 그룹별 통계 | Chapter 6 (1.3절 예습) |
| scikit-learn[^scikitlearn] | 분류·회귀·클러스터링 모델의 표준 인터페이스 | Chapter 2 |
| PyTorch[^pytorch] (deep learning) | 신경망·CNN·시퀀스 모델·VAE/GAN | Chapter 9 |
| Hugging Face Transformers[^huggingface] | LLM 프롬프팅, 사전학습 모델 활용 | Chapter 17 |
| gymnasium[^gymnasium] | (ML2에서) 강화학습 환경. ML1에서는 등장하지 않음 | — |
...
첫 학기에 가장 흔한 실패 패턴은, Chapter 12의 Transformer나 Chapter 17의
...
적용되고(Chapter 9의 드롭아웃[^dropout], 배치정규화[^batchnorm]가 정확히 같은 원리다), 지금도
...
[^diffusion]: Ho, J., Jain, A., Abbeel, P. (2020). "Denoising Diffusion Probabilistic Models." arXiv:2006.11239.
[^cs229]: Stanford CS229: Machine Learning, Lecture Notes. https://cs229.stanford.edu/main_notes.pdf
[^dpo]: Rafailov, R., Sharma, A., Mitchell, E., Ermon, S., Manning, C. D., Finn, C. (2023). "Direct Preference Optimization: Your Language Model is Secretly a Reward Model." arXiv:2305.18290.
...
[^huggingface]: Wolf, T., Debut, L., Sanh, V., et al. (2019). "HuggingFace's Transformers: State-of-the-art Natural Language Processing." arXiv:1910.03771.
[^pytorch]: Paszke, A., Gross, S., Massa, F., et al. (2019). "PyTorch: An Imperative Style, High-Performance Deep Learning Library." arXiv:1912.01703.
