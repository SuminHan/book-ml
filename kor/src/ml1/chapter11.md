...
"순차적 압축 자체의 한계"를 극복하러 Chapter 12의 어텐션으로 이어진다.
...
반복 곱해진다. 이 두 가지가 이 장의 그래디언트 소실(11.3)이자, Chapter 12가
...
- 그래디언트 곱적이 W_hh의 고유값이 1보다 작으면 지수적으로 0으로
  ...
  조절 스위치, 덧셈은 통로"로 설계된 ResNet 스킵 연결의 시간 축 버전을
  원리 수준으로 서술한다.
- 실전 상황에서 기본 RNN → GRU → LSTM → Attention 사이에서 언제 무엇을
