...
## 11.5 그래디언트 소실이 시간 축에서 재발하는 이유

Chapter 9.4에서 본 것과 정확히 같은 패턴이다: \\(\tanh'\\)의 최댓값은
...
10.9에서 본 ResNet의 스킵 연결(\\(y=F(x)+x\\))과 정확히 같은 원리다. 덧셈은
