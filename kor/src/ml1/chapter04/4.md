...
[STUB: cq 확인 필요 — 원래 15.1(잠재변수 생성모델 챕터)에 있던 내용을
4장(거리 기반 모델과 클러스터링) 끝으로 옮겨왔다. **사용자 요청 반영
필수**: 이 절에 **LDA(잠재 디리클레 할당, Latent Dirichlet
Allocation)**도 GMM/EM과 같은 "EM으로 푸는 고전 확률모델"로 추가할
것 — 문서-토픽 모델이라는 점에서 GMM(포인트-클러스터)과 대비해서
설명하면 좋음. 절 제목도 필요하면 "GMM, EM 알고리즘, 그리고 LDA"
식으로 확장 검토. 15장(VAE/GAN/Diffusion) 오프너에서 "이 절이 그
확률적 잠재변수 모델의 원형"이라고 다시 짚어주는 다리 문단이 있으니
호응 확인할 것.]
