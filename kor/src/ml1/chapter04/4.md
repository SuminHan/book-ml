...
4.3절의 k-means는 각 점을 **딱 하나의** 클러스터에 배정했다(하드
...
하나였음을 보인 사건이다. 다음 장(Chapter 15)의 주제인 "관측되지 않은
...
### EM 알고리즘[^dempster1977]: E-step과 M-step
...
**ELBO**(Evidence Lower BOund)라는 이름으로 다음 장(Chapter 15.1절)에서
...
\\[\log p(x) \ge \mathbb{E}\_{z \sim \gamma}[\log p(x, z)] - \mathbb{E}\_{z \sim \gamma}[\log \gamma(z)] \\;\\; \stackrel{\text{def}}{=} \\; \mathcal{F}(\theta, \gamma)\\]
...
절대 줄지 않는다**. 다음 장 15.1절의 VAE[^vae]는 "E-step의 사후확률을 정확히
...
soft assignment의 정수다 — k-means에서는 이 점이 임의로 한쪽
...
### 같은 EM, 다른 데이터: LDA(잠재 디리클레 할당)

GMM을 생성 과정의 관점에서 다시 보면, "데이터가 **어디서** 만들어졌는가"를
모델링한 것이다 — 각 점 \(x\_i\)의 생성 클러스터(어느 덩어리)를
나타내는 잠재변수 \(z\_i\)가 한 줄기뿐이다. 그런데 데이터가 점으로
만들어지는 것은 아니다. 문서(뉴스 기사, 논문, 리뷰)의 경우 "이 문서가
**어떤 주제로** 만들어졌는가"를 물으면, 잠재변수는 한 줄기가 아니라
**K개 주제의 혼합 비율** \(\theta\_d\)(\(\theta\_{dk}\)는 문서
\(d\)가 주제 \(k\)를 쓰는 비중, \(\sum\_k \theta\_{dk} = 1\))가
된다. 이 질문을 같은 EM의 틀로 푸는 것이 **LDA**(Latent Dirichlet
Allocation, 잠재 디리클레 할당)[^lda]다 — 2003년, 다비드 블레이(David
Blei), 앤드류 응(Andrew Ng), 마이클 조던(Michael Jordan)의 논문이다.
이름의 'Dirichlet'은 아래에서 나오는 사전분포의 이름이고,
'Allocation'은 '할당'을 뜻한다 — 문서가 토픽에 할당되고, 토픽이 단어에
할당된다는 생성 과정을 가리킨다.

**생성 과정**은 GMM과 골격이 그대로 같다 — "주사위를 굴리고, 그
주사위 결과의 분포에서 샘플링한다"는 두 단계다:

1. **토픽 혼합 뽑기**: 문서 \(d\)의 토픽 혼합 비율 \(\theta\_d\)를
   디리클레 사전분포 \(\text{Dir}(\alpha)\)에서 뽑는다
   (\(K\)개 주제에 대한 확률벡터 — GMM의 혼합비율 \(\pi\)에
   대응).
2. **단어 뽑기**: 문서의 각 단어 위치 \(n\)마다, (i) 주사위
   \(\text{Categorical}(\theta\_d)\)로 토픽 \(z\_{dn}\)를 뽑고,
   (ii) 그 토픽의 단어 분포 \(\phi\_{z\_{dn}}\)에서 단어
   \(w\_{dn}\)를 뽑는다.

여기서 \(\phi\_k\)는 "주제 \(k\)의 단어 프로필"이다 — '정치'
토픽이라면 "선거·정책·대통령" 같은 단어가, '경제' 토픽이라면
"금리·물가·주식" 같은 단어가 높은 확률을 갖는 어휘 크기만큼의
확률벡터. \(\phi\_k\) 자체도 디리클레 사전분포
\(\text{Dir}(\beta)\)에서 뽑힌다. 두 사전분포의 파라미터
\(\alpha, \beta\)는 "문서가 주제를 어떻게 섞는지", "토픽이 단어를
어떻게 섞는지"에 대한 **사전적 믿음**을 담은 하이퍼파라미터이며,
통상 균일한 작은 값(예: \(\alpha = 50/K,\ \beta = 0.01\))을 쓴다.
디리클레 분포는 단순히 "확률벡터 위의 분포"가 아니라 **다항분포의
켤레(conjugate) 분포**라 E-step과 M-step이 전부 닫힌 형태로 유지된다
— 이 "켤레성의 선물"은 GMM의 정규-감마 구조가 해준 것과 정확히 같은
역할이다.

**E-step과 M-step**은 "책임값이 무엇으로 바뀌는가"만 다를 뿐이다:

- **E-step**: 문서 각 단어 위치의 토픽 사후확률
  \(\gamma\_{dnk} = P(z\_{dn} = k \mid w\_d, \theta, \phi)\)를
  계산한다. GMM의 \(\gamma\_{ik}\)가 "점 \(i\)가 클러스터
  \(k\)일 확률"이라면, \(\gamma\_{dnk}\)는 "문서 \(d\)의
  \(n\)번째 **단어**가 토픽 \(k\)에서 왔을 확률"이다 — 관측
  데이터가 점 대신 **단어**가 된 것뿐이다.
- **M-step**: 책임값으로 두 파라미터를 갱신한다 —
  \(\theta\_{dk} \propto \sum\_n \gamma\_{dnk}\)(이 문서가
  토픽 \(k\)를 쓰는 비중), \(\phi\_{kw} \propto
  \sum\_{d,n : w\_{dn} = w} \gamma\_{dnk}\)(토픽 \(k\)에서
  단어 \(w\)가 나온 누적 책임). GMM M-step의 "책임값 가중 평균"과
  동일한 구조다.

이 반복은 엄밀히 말해 "완전한 EM"이 아니다 — E-step이 각 단어
위치의 토픽을 **독립**으로 근사하는 변분 추론(variation
inference, mean-field 근사)이라, 정석 명칭은 변분 EM(VEM)이다.
"계산 안 되는 E-step을 근사한다"는 수법은 Chapter 15.1절의
VAE[^vae]가 신경망으로 하는 것의 **가장 고전적인 예시**다 — VAE가
신경망으로 근사하는 것을, LDA는 "위치별 독립"이라는 더 단순한
가정으로 근사한 셈이다.

**GMM(포인트-클러스터)과의 대비**를 한 표로 보자:

| | GMM | LDA |
|---|---|---|
| 관측 데이터 | 점 \(x\_i \in \mathbb{R}^d\) | 문서의 단어 \(w\_{dn}\)(이산) |
| 잠재변수 | 클러스터 라벨 \(z\_i\)(1개/점) | 토픽 \(z\_{dn}\)(1개/단어) + 혼합 \(\theta\_d\)(1개/문서) |
| "덩어리"의 모양 | 정규분포 \(\mathcal{N}(\mu\_k, \Sigma\_k)\) | 단어 분포 \(\phi\_k\)(디리클레 사전 위) |
| "클러스터"의 의미 | 특징공간에서 가까이 모인 점 | 자주 함께 나타나는 단어 패턴(토픽) |
| M-step 갱신 | \(\mu\_k, \Sigma\_k, \pi\_k\) | \(\theta\_d, \phi\_k\) |

"클러스터"가 특징공간에서의 **기하학적 근접성**(GMM)이 아니라
**단어 공출현의 통계적 패턴**(LDA)으로 바뀐 것 — 이것이 같은 EM의
두 가지 얼굴을 가질 수 있는 이유다. \(K\) 선택 문제도 그대로
이어진다(토픽 수를 고르는 것도 BIC 계열의 트레이드오프 — 다만
우도를 닫힌 형태로 계산할 수 없어 변분 하한을 기준으로 삼는다).
실전에서는 LDA 계열 토픽모델로 뉴스 분류·의견 분석·문서 자동
요약을 만들고, 14.3절의 문서 임베딩·RAG 시스템에서 "어떤 문서가
관련 있는가"를 판정하는 보조 도구로 쓰이기도 한다.

**자주 묻는 질문 — "LDA"라는 이름이 이 책에서 두 번 등장하는 것
아니냐?** 맞다 — Chapter 3.2에서 GDA와 같은 모델로 나온 **피셔
선형 판별(Fisher's Linear Discriminant)도 LDA**다 — 서로 다른
문제를 푸는 별개의 모델이 나중에 약자를 공유하게 된 것이다.
토픽모델의 LDA는 2001년 NIPS에서 발표된 Blei, Ng, Jordan의 논문
(원형은 그 NIPS 2001 버전이고, 이 책에서는 확장된 JMLR 2003
버전을 인용한다)에서 붙은 이름이다. 이 절에서 LDA라 하면 전부 "잠재 디리클레
할당"을 뜻하며, 차원축소/분류의 LDA는 3.2절의 문맥에서만
등장한다.
...
[노트북](https://colab.research.google.com/github/SuminHan/book-ml/blob/main/notebooks/ml1/chapter04_4_gmm_em.ipynb)
...
풀린다. VAE(다음 장 15.1절)의 잠재변수는 **연속** 벡터라 가능한 \\(z\\)가
...
샘플링하면 된다. 이 "생성" 능력이 다음 장(Chapter 15)
...
**5. (손유도, Tier C — 폴백 준비 대상)** 다음 장 15.1절 ELBO
...
을 보여준 것이다 — 다음 장 15.1절의 VAE는 E-step을 정확히 계산할 수
...
[^dempster1977]: Dempster, A. P., Laird, N. M., Rubin, D. B. (1977). "Maximum Likelihood from Incomplete Data via the EM Algorithm." Journal of the Royal Statistical Society: Series B 39(1), 1–22. — EM 알고리즘을 일반적인 형태로 정식화한 원 논문.
[^lda]: Blei, D. M., Ng, A. Y., Jordan, M. I. (2003). "Latent Dirichlet Allocation." Journal of Machine Learning Research 3(Jan), 993–1022. https://jmlr.org/papers/v3/blei03a.html — 문서를 "토픽의 혼합"으로 표현하고 변분 EM으로 학습하는 토픽 모델의 원 논문(원형은 NIPS 2001, 601–608).
