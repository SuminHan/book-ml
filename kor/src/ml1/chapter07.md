# Chapter 7. 신경망 기초와 역전파 (Neural Network Basics & Backpropagation)
...
학습시키는가"다 — 출력층은 Chapter 3의 로지스틱회귀와 똑같이 그래디언트를
...
1. **출력층 오차**: \\(\delta_2 = \frac{\partial L}{\partial z_2} = (a_2 - y)
   \sigma'(z_2)\\) (Chapter 3에서 본 것과 같은 형태 — 교차 엔트로피와
...
사라지는(vanishing gradient, Chapter 8에서 다룸) 문제를 마주쳤을 때 원인을
...
**2. (손유도, Tier C — 폴백 준비 대상)** 위 신경망에 대해 손실함수
