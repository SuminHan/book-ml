...
[STUB: cq 확인 필요 — 이 절 안에 "네 원리(EM/GMM·VAE·GAN·Diffusion)
비교" 표/문단이 있으면 EM/GMM을 빼고 "세 원리(VAE·GAN·Diffusion)"로
줄일 것. Diffusion 부분에 Waymo SceneDiffuser[^scenediffuser] 실전
사례(자율주행 시뮬레이션 시나리오 생성) 짧게 추가.]

[![Open In Colab: GAN & Diffusion](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/SuminHan/book-ml/blob/main/notebooks/ml1/chapter15_3_gan_diffusion.ipynb)

앞의 두 절은 같은 질문("관측되지 않은 구조가 데이터를 만들어내는가")을
두 가지 방식으로 풀었다 — GMM은 EM 알고리즘으로 정확히 풀었고, VAE[^vae]는
풀리지 않는 ELBO를 신경망으로 근사했다. 이번 절은 그 다음, **우도
기반과 확연히 다른** 나머지 두 방식을 만난다. GAN은 두 신경망의
**경쟁 게임**으로 학습하고, Diffusion은 노이즈를 **한 단계씩 되돌리는
회귀 문제**로 학습한다. 네 방식이 나란히 놓이면 Chapter 15 전체의
그림이 완성된다(생성 모델 전반을 더 깊이 보려면
[^cs230]도 참고하라).
...
15.1절 EM의 뼈대(E-step: 파라미터 고정 → 잠재변수 채우기 /
...
여기서 EM과의 차이를 한 번 더 명확히 하자. EM은 E-step이 **정확한
...
반대로 VAE(15.2절)에서는 KL 항 \\(D\_{KL}(q\_\phi(z|x) \\| p(z))\\)
...
없다 — 이것이 15.3의 GAN 섹션에서 VAE와 비교할 때의 **핵심
구조적 차이**다.
...
- **StyleGAN (2018–2019)**[^stylegan]: 초고해상도 합성 인간 얼굴 생성.
  ...
  해졌다. (이 방향이 16장의 "생성 모델의 응용"에서 다시
  나온다.)
...
여기에 15.2절의 **VAE**를 조합했다 — 이미지를 VAE 인코더로
8× 압축한 **잠재 공간**에서 Diffusion을 돌리는 구조로,
계산량이 1/8²으로 줄었다. 15.2에서 "VAE의 매끄러운 잠재
공간"이 왜 중요한지 배웠는데, 그것이 곧 Diffusion을 **안정
있게** 학습·추론할 수 있게 하는 전제 조건이었다 — 네 원리가
각자 독립적으로 끝나는 것이 아니라 **서로 연결**되어 실전
시스템으로 합쳐진다는 점이, 이번 장을 "네 개를 나란히" 보는
이유다.
...
**잠재변수 행을 다시 읽으면** "관측 안 된 구조"가 네 방식에서
무엇이었는지 보인다. EM/GMM의 \\(z\\)는 **이산 라벨**(어느
클러스터인가) — 가능치가 \\(K\\)개라 E-step이 닫힌 형태로
**정확히** 계산된다. VAE의 \\(z\\)는 **연속 벡터** — 가능치가
무한해서 E-step을 신경망으로 **근사**해야 한다(15.2절). GAN의
